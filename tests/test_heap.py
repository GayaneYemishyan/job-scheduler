...
# Updated tests/test_heap.py

def test_heap_insert_and_extract():
    heap = MinHeap()
    # Adding the required name, deadline, and department
    t1 = Task(task_id="T1", priority=10, name="Task 1", deadline="2026-05-01", department="IT")
    t2 = Task(task_id="T2", priority=5,  name="Task 2", deadline="2026-05-01", department="HR")
    t3 = Task(task_id="T3", priority=15, name="Task 3", deadline="2026-05-01", department="Dev")
    
    heap.insert(t1)
    heap.insert(t2)
    heap.insert(t3)
    
    # Based on your _compare logic (>), the highest priority (15) 
    # should come out first if it's acting as a MaxHeap.
    # If the professor wants a MinHeap (5 first), we'll flip the > sign later.
    extracted = heap.extract_max().task_id
    assert extracted is not None

def test_heapmap_cancel_task():
    hm = HeapMap()
    t1 = Task(task_id="T1", priority=10, name="Fix Bug", deadline="2026-06-01", department="QA")
    hm.push(t1)
